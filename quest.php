...
if($saveid>0) {
	...
	for($i=1;$i<=50;$i++) if($planet->feld[$i]->was==$gebaude && $planet->feld[$i]->bauzeit==0) { $gefunden=true; $bcount++; }
...
$sid=$_GET["sid"];
if(!ctype_digit($sid)) die();
$uid=$_GET["uid"];
if(isset($_GET["uid"]) && !ctype_digit($sid)) die();

if($uid>0) $ushp=new Schiffe($uid);

if($sid>0) { $schiff=new Schiffe($sid);
if($sid>0 && ( $ushp->position->x!=$schiff->position->x || $ushp->position->y!=$schiff->position->y || $ushp->position->orbit!=$schiff->position->orbit || $ushp->position->system->id!=$schiff->position->system->id)) die("interner Questfehler. bitte Admin benachrichtigen!<br />");
...
if(isset($_GET["aid"]) && ctype_digit($_GET["aid"]))
		...
		if($qcounter>=$neuq->max) 
			...
			*/
			...
			if($result) echo "Quest erfolgreich abgegeben.";
				...
				echo "Hinweis: Schiff ".$shp3->name." (".$shp3->id.") ist zwar vor Ort hat aber keine ".$neuq->max." ".$qit->name." an Board!<br />"; $schiff_bekommen=false; }
		}
		}
	} else {
	if($neuq->typ==2) {
	...
	/* QUESTBELOHNUNG */
		...
		if($neuq->brohstoffe[0]!='' && $neuq->abgeber>0) for($i=0;$i<sizeof($neuq->brohstoffe);$i++)
			...
			//schiffsberechnung
			$ranzahl=$roh[1];
			if($ushp->frachtraum->gesamt()+$ranzahl>$ushp->frachtraum->max) $ranzahl=$ushp->frachtraum->max-$ushp->frachtraum->gesamt();
			if($ranzahl<0) $ranzahl=0;
...
if( ($fertig==-1 && $schiff->id==$row["geber"] && $uid>0) || ($row["geber"]==0 && $fertig==-1) || ($row["abgeber"]==0 && $fertig>=0) || ($fertig>=0 && $schiff->id==$row["abgeber"] && $uid>0) )  {
	...
	$lvl=new Account('5');
	echo '<div class="rahmen">Questgeber: '; if($sid>0) echo $schiff->besitzer->nickname; else echo $lvl->nickname; echo '<hr /><br />';
	echo '<h3>',nl2br($row["titel"]),'</h3>';
	if($fertig>=1) echo '',nl2br($row["abgabetext"]),'<br /><hr />';
	else					echo '',nl2br($row["text"]),'<br /><br /><hr />';
	if($fertig==-1) echo '<a href="quest.php?uid=',$_GET["uid"],'&sid=',$sid,'&qid=',$row["id"],'"><span style="color:yellow;font-weight:bold;">annehmen</span></a></div><br />';
	if($fertig==1) echo '<a href="quest.php?uid=',$_GET["uid"],'&sid=',$sid,'&aid=',$aid,'"><span style="color:yellow;font-weight:bold;">Quest beenden</span></a></div><br />';
	if($fertig==0) echo '<span style="color:red;font-weight:bold;">noch nicht abgeschlossen</span></div><br />';
	if($fertig==2) echo '<span style="color:green;font-weight:bold;">bereits abgeschlossen</span></div><br />';
	}
	}
	}
echo '<br />';

if($uid>0) echo '<br /><a href="schiffe.php?sid=',$uid,'"><span class="uberschrift">zur&uuml;ck zum Schiff</span></a>';
include("foot.php");
?>
