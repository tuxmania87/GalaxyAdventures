    ...
    if (in_array($neuq->abgeber, $sarray) || $neuq->abgeber == 0) {
        ...
        if ($neuq->typ == 6) {
            ...
            if ($neuq->abgeber == 0) {
                ...
                $result = mysqli_query($verbindung, "UPDATE erfolge SET erledigt=2 WHERE erledigt=1 AND id='".intval($_GET['aid'])."' AND uid='".intval(\$_SESSION['Id'])."'");
                if ($result) {
                ...
                $abfrage = mysqli_query($verbindung, "SELECT * FROM schiffe WHERE x='".$shp2->position->x."' AND y='".$shp2->position->y."' AND orbit='".$shp2->position->orbit."' AND system='".$shp2->position->system->id."' AND besitzer='".intval(\$_SESSION['Id'])."'");
                while ($row = mysqli_fetch_array($abfrage)) {
                    // auswahl einengen alle schiffe die max anzahl an qitems dabei haben in $preship werfen (id)
                    $qcounter = 0;
                    $stellen = [];
                    $shp3 = new Schiffe($row['id']);
                    for ($i = 0; $i < count($shp3->qitems); ++$i) {
                    ...
                    if ($qcounter >= $neuq->max) {	// abgeben
                        // for($k=0;$k<count($stellen);$k++) unset($shp3->qitems[$stellen[$k]]);
                        /*
            ...
            if ($neuq->qid == 18) {
                mysqli_query($verbindung, "UPDATE forschung SET oberth=1 WHERE besitzer='".intval(\$_SESSION['Id'])."'");
            }
        ...
        if ($neuq->qid == 30) {
            mysqli_query($verbindung, "UPDATE forschung SET miranda=1 WHERE besitzer='".intval(\$_SESSION['Id'])."'");
        }

        /* QUESTBELOHNUNG */
        if ((count($neuq->bschiffe) > 0 || count($brohstoffe) > 0) && $schiff_bekommen) {
            $heimat = mysqli_query($verbindung, "SELECT x,y,`system` FROM planeten WHERE heimat=1 AND besitzer='".intval(\$_SESSION['Id'])."'");
            $heimat = mysqli_fetch_array($heimat);
            if ($neuq->bschiffe[0] != '') {
                for ($i = 0; $i < count($neuq->bschiffe); ++$i) {
                    $lastid = checkforlastid('schiffe') + 1;
                    echo 'MILCH: ',$neuq->bschiffe[$i],'<br />';
                    mysqli_query($verbindung, "INSERT INTO schiffe (name,x,y,`system`,besitzer,energie,maxenergie,energieoutput,maxwarpkern,schilde,maxschilde,hull,maxhull,bild,klasse,typ,lager,deuterium,skilldeut,skillerz,skilltranswarp,laser,maxtorpedo,maxgondeln,maxphaser,skilltarnung,lrs,skillbau) SELECT 'noname','".$heimat[0]."','".$heimat[1]."','".$heimat[2]."','".intval(\$_SESSION['Id'])."',maxenergie,maxenergie,energieoutput,maxwarpkern,maxschilde,maxschilde,maxhull,maxhull,bild,klasse,typ,lager,'40',skilldeut,skillerz,skilltranswarp,laser,maxtorpedo,maxgondeln,maxphaser,skilltarnung,lrs,skillbau FROM bauplan WHERE klasse='".$neuq->bschiffe[$i]."'") or exit(mysqli_error($verbindung));
            ...
            if ($neuq->brohstoffe[0] != '' && $neuq->abgeber > 0) {
                for ($i = 0; $i < count($neuq->brohstoffe); ++$i) {
...
if (isset($_GET['qid']) && ctype_digit($_GET['qid'])) {
    ...
    $abfrage = mysqli_query($verbindung, "SELECT typ,zusatz FROM quests WHERE id='".intval($_GET['qid'])."'");
    $abfrage = mysqli_fetch_array($abfrage);
    $qtyp = $abfrage[0];
    $zusatz1 = $abfrage[1];
    $abfrage = mysqli_query($verbindung, "SELECT id FROM planeten WHERE besitzer='".intval(\$_SESSION['Id'])."'");
    while ($row = mysqli_fetch_array($abfrage)) {
    ...
    // quest annehmen
    $abf = mysqli_query($verbindung, "SELECT * FROM erfolge WHERE qid='".intval($_GET['qid'])."' AND uid='".intval(\$_SESSION['Id'])."'");
    while ($ro = mysqli_fetch_array($abf)) {
        exit('Quest bereits vorhanden!');
    }
    if ($qtyp != 4 && $qtyp != 8) {
        mysqli_query($verbindung, 'INSERT INTO erfolge (uid,qid,anzahl) VALUES ('.intval(\$_SESSION['Id']).','.intval($_GET['qid']).',0)') or exit($verbindung->error);
    }
    if ($qtyp == 4) {
        mysqli_query($verbindung, "INSERT INTO erfolge (anzahl,uid,qid) VALUES ('$bcount','".intval(\$_SESSION['Id'])."','".intval($_GET['qid'])."')") or exit(mysqli_error($verbindung));
    }
    if ($qtyp == 8) {
        mysqli_query($verbindung, "INSERT INTO erfolge (erledigt,uid,qid) VALUES ('1','".intval(\$_SESSION['Id'])."','".intval($_GET['qid'])."')") or exit(mysqli_error($verbindung));
    }
    if ($qtyp == 8) {
...
while ($row = mysqli_fetch_array($abfrage)) {
    ...
    $ch = false;
    $abfrage2 = mysqli_query($verbindung, "SELECT * FROM erfolge WHERE uid='".intval(\$_SESSION['Id'])."' AND erledigt=2 AND qid='".$row['vorquest']."'");
    ...
    if ($ch || $row['vorquest'] == 0) {
        // ende der beschr
        $tempid = $row['id'];
        $abfrage2 = mysqli_query($verbindung, "SELECT * FROM erfolge WHERE uid='".intval(\$_SESSION['Id'])."' AND qid='".$tempid."'");
        $fertig = -1;
        while ($row2 = mysqli_fetch_array($abfrage2)) {
        ...
        if ($row['typ'] == 2 && $uid > 0 && $fertig == 0) {
            ...
            if ($ushp->frachtraum->$tvar >= $row['max']) {
                $fertig = 1;
                mysqli_query($verbindung, "UPDATE erfolge SET erledigt=1 WHERE qid='".$row['id']."' AND uid='".intval(\$_SESSION['Id'])."' AND erledigt=0");
                $row['erledigt'] = 1;
            }
        }
