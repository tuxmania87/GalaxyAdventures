...
//fid abfrage
$planet->xx = 'feld' . $fid;
$abfrage11 = mysql_query("SELECT * FROM planet2 WHERE pid='$pid'");
while ($test11 = mysql_fetch_array($abfrage11)) {
    ...
    if ($slots >= 50) {
        ...
        if(isset($_POST["pschiff"])) {
            ...
            if($buildit && $p->energie >= $b->kosten->fracht[0]->anzahl) {
                ...
                mysql_query("insert into schiffe 
                ...
                mysql_query("update planeten set energie='".$p->energie."' where id = ".$p->id);
                $p->frachtraum->save();
...
// frachtraum
        ...
        echo '<h3>Schiffsbau - Werft</h3>';
        
        //check if already building
        $q = mysql_query("select klasse from schiffe where typ='' and nachricht='".$pid."/".$fid."'");
        ...
        echo '<form action="createship?pid='.$pid.'&fid='.$fid.'" method="post">';
        ...
        $q = mysql_query("select klasse from bauplan where siedler = 1");
