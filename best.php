...
function tausche(&$a,&$b) { $h=$a; $a=$b; $b=$h; }
...
$i=0;
$abfrage=mysql_query("SELECT id,wpunkte FROM account WHERE id>9 ORDER BY wpunkte DESC");
...
$playerresult=mysql_query("SELECT * FROM account WHERE id>9");
while($player=mysql_fetch_array($playerresult)) {

$id=$player["id"];
$gesamt=0;
$sammelquery=mysql_query("SELECT * FROM schiffe WHERE besitzer='$id' AND typ='s'");
while($sammel=mysql_fetch_array($sammelquery)) {
if($sammel["klasse"]=='Oberth') $gesamt+=2;
if($sammel["klasse"]=='Miranda') $gesamt+=5;
if($sammel["klasse"]=='Constitution') $gesamt+=10;
}
if($gesamt>0) {
$idfeld[]=$id;
$army[]=$gesamt; }
