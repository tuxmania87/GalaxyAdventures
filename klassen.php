...
class Quest
{
    ...
    {
        $verbindung = get_verbindung();
        $abfrage = mysqli_query($verbindung, "SELECT * FROM erfolge,quests WHERE erfolge.id='" . $qqid . "' AND quests.id=erfolge.qid") or die(mysql_error());
...
class Logbuch
{
    ...
    {
        $verbindung = get_verbindung();
        mysqli_query($verbindung, "UPDATE logbuch SET zeit='$this->zeit',klasse='$this->klasse',typ='$this->typ',initiator='$this->initiator',betroffener='$this->betroffener',x='$this->x',y='$this->y',system='" . $this->system->id . "',text='$this->text' WHERE id='$this->id'") or die(mysql_error());
    }
}
...
class Channel
{
    ...
    public static function getList()
    {
        $l = array();
        $verbindung = get_verbindung();
        $q = mysqli_query($verbindung, "select id from channel where public=1 or id in (select cid from channelabo where status=1 and uid='" . session_id() . "') or   founder = " . $_SESSION["Id"] . "            order by id") or die(mysqli_error($verbindung));
        while ($r = mysqli_fetch_array($q)) {
...
class Frachtraum
{
    ...
    public function __construct($sid, $typ)
    {
        ...
        if (ctype_digit($sid) || $typ == 'dummy') {
            ...
            if ($typ == 'planet') {
                print $sid . " " . $typ . "<br>";
                $abfrage = mysqli_query($verbindung, "SELECT maxfrachtraum,lager,frachtraum FROM planeten p WHERE id='$sid'") or die(mysqli_error($verbindung));
            }
            if ($typ == 'konto')
...
class System
{
    ...
    public function __construct($sid)
    {
        ...
        if (ctype_digit($sid)) {
            $abfrage = mysqli_query($verbindung, "SELECT * FROM systeme WHERE id='$sid'") or die(mysql_error());
            while ($row = mysqli_fetch_array($abfrage)) {
                $this->id = $sid;
                $this->name = $row["name"];
            ...
            $abfrage = mysqli_query($verbindung, "SELECT * FROM systeme WHERE x='" . $sid[0] . "' and y='" . $sid[1] . "'") or die(mysql_error());
            while ($row = mysqli_fetch_array($abfrage)) {
                $this->id = $row["id"];
                $this->name = $row["name"];
                $this->feld = new Systemfelder($row["typ"]);
    ...
    public function feuern($ziel, $modus)
    {
        ...
        if ($modus == 1 || $modus == 11 || $modus == 21) {
            ...
            if ($this->besitzer->id != $ziel->besitzer->id && $ziel->besitzer->id != 2 && $this->besitzer->id != 2 && $this->energie >= 1 && (($this->phaser < $this->maxphaser && $modus == 1) || ($this->torpedohitze < $this->maxtorpedohitze && $modus == 11) || ($this->torpedohitze < $this->maxtorpedohitze && $modus == 21)) && $this->tarnung == 0 && $this->position->x == $ziel->position->x && $this->position->y == $ziel->position->y && $this->position->orbit == $ziel->position->orbit && $this->position->system->id == $ziel->position->system->id && (($this->besitzer->allianz->id != $ziel->besitzer->allianz->id) || $this->besitzer->allianz->id == 0) && !in_array($this->besitzer->id, $ziel->besitzer->vertrag("nap")) && !in_array($this->besitzer->id, $ziel->besitzer->vertrag("frieden")) && (($ziel->besitzer->level >= 4 && $this->besitzer->level >= 4) || $ziel->klasse = 'Vorposten')) {
                ...
                if ($ziel->typ == 's')
                    mysqli_query($verbindung, "UPDATE schiffe SET dock='$ziel->dock',phaser='$ziel->phaser',energie='$ziel->energie',hull='$ziel->hull',schilde='$ziel->schilde',schildstatus='$ziel->schildstatus' WHERE id='$ziel->id'") or die(mysql_error());
                else
                    mysqli_query($verbindung, "UPDATE planeten SET phaser='$ziel->phaser',energie='$ziel->energie',hull='$ziel->hull',schilde='$ziel->schilde',schildstatus='$ziel->schildstatus' WHERE id='$ziel->id'") or die(mysql_error());
...
class Schiffe extends Rohling
{
    ...
    public function zerstoerung()
    {
        ...
        if (mysqli_num_rows($abfrage) > 0) {
            ...
            if ($this->frachtraum->gesamt() > 0) {
                ...
                mysqli_query($verbindung, "insert into schiffe (energie,hull,warpkern,frachtraum,name,x,y,typ,system,klasse,besitzer) values ('" . $t_fracht->maxenergie . "','" . $t_fracht->maxhull . "','" . $t_fracht->maxwarpkern . "','" . $this->frachtraum->dump() . "','verlorene Fracht','" . $randx . "','" . $randy . "','s','" . $this->position->system->id . "','" . $t_fracht->klasse . "','2')") or die(mysql_error());
            }
        }
        mysqli_query($verbindung, "UPDATE schiffe SET dock=0 WHERE dock='$this->id'");
        mysqli_query($verbindung, "UPDATE schiffe SET loot=1,klasse='$this->klasse',hull='$this->hull',name='$this->name',besitzer=" . $this->besitzer->id . ",schildstatus='$this->schildstatus',schilde='$this->schilde',frachtraum='' WHERE id='$this->id'") or die(mysql_error());
        return 0;
    }
    ...
    public function navigieren($richtung, $flotte, $test)
    {
        ...
        if ($test == 0) {
            ...
            mysqli_query($verbindung, "UPDATE schiffe SET gondeln='" . $this->gondeln . "',energie='" . $this->energie . "',system='" . $this->position->system->id . "',x=" . $this->position->x . ",y=" . $this->position->y . ",orbit=" . $this->position->orbit . " WHERE id='$this->id'") or die(mysql_error());
            mysqli_query($verbindung, "UPDATE counter SET count=count+1 WHERE id=1") or die(mysql_error());
...
class Bauplan_Gebaude
{
    ...
    public function __construct($id)
    {
        ...
        $verbindung = get_verbindung();
        $q = mysqli_query($verbindung, "select * from gebaude where id=" . $id) or die(mysqli_error($verbindung));
        while ($r = mysqli_fetch_array($q)) {
            $this->id = $id;
            $this->name = $r["name"];
