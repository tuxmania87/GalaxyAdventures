...
if ($betray) {
    ...
    function feuern(&$angreifer, &$verteidiger, $fid)
    {
        ...
        if ($fehler != '') {
            ...
            --$angreifer->energie;
            ++$angreifer->phaser;
            mysqli_query($verbindung, "UPDATE schiffe SET energie=energie-1,phaser=phaser+1 WHERE id='$angreifer->id'");
            if ($verteidiger->feld[$fid]->hull - $amount > 0) {
                ...
                if ($verteidiger->feld[$fid]->was == 2) {
                    ...
                    mysqli_query($verbindung, "UPDATE planeten SET lager=lager-500,maxenergie=maxenergie-20 WHERE id='$verteidiger->id'");
                }
                if ($verteidiger->feld[$fid]->was == 5 || $verteidiger->feld[$fid]->was == 17) {
                    $verteidiger->laser -= 6;
                    mysqli_query($verbindung, "UPDATE planeten SET laser=laser-6 WHERE id='$verteidiger->id'");
                }
                if ($verteidiger->feld[$fid]->was == 6 || $verteidiger->feld[$fid]->was == 16) {
                    ...
                    } mysqli_query($verbindung, "UPDATE planeten SET schilde='$verteidiger->schilde',maxschilde='$verteidiger->maxschilde' WHERE id='$verteidiger->id'");
                }
                if ($verteidiger->feld[$fid]->was == 3 || $verteidiger->feld[$fid]->was == 7 || $verteidiger->feld[$fid]->was == 13) {
                    ...
                    } mysqli_query($verbindung, "UPDATE planeten SET energie='$verteidiger->energie',maxenergie='$verteidiger->maxenergie' WHERE id='$verteidiger->id'");
                }
                if ($verteidiger->feld[$fid]->was == 12) {
                    $verteidiger->lager -= 200;
                    mysqli_query($verbindung, "UPDATE planeten SET lager=lager-200 WHERE id='$verteidiger->id'");
                }
    ...
    if ($schiff->besitzer->id == 2) {
        exit('Dein Schiff wurde zerst&ouml;rt');
    }
    // preufen auf reigabe
    $pcounter = 0;
    $pwas = 0;
    ...
    if ($pcounter == 0) {
        mysqli_query($verbindung, "UPDATE planeten SET besitzer=13 WHERE id='".$planet->id."'");
