...
if($feld->was=="System") die("Bauen auf Systemfeldern nicht erlaubt!");


//nebel test
$nebel=false;
$nebelvar=mysql_query("SELECT * FROM schiffe WHERE (typ='b' OR typ='g') AND x='".$schiff->position->x."' AND y='".$schiff->position->y."'");
while($testb=mysql_fetch_array($nebelvar)) { $nebel=true; }
$feindpl=false;
$nebelvar=mysql_query("SELECT * FROM schiffe WHERE typ='m' AND x='".$schiff->position->x."' AND y='".$schiff->position->y."' AND orbit='".$schiff->position->orbit."' AND besitzer!='$self'");
while($testb=mysql_fetch_array($nebelvar)) { $feindpl=true; }


if($nebel) echo "Du kannst nicht im Nebel bauen!"; else if ($feindpl) echo "Du kannst nicht im Orbit von fremden Planeten bauen!"; else {
...
$lastid=checkforlastid('schiffe')+1;
mysql_query("INSERT INTO schiffe (klasse,skillbase,id,baustoff,duranium,deuterium,hull,maxhull,schilde,maxschilde,laser,name,besitzer,alarmstufe,x,y,system,energie,maxenergie,energieoutput,bild,lager,orbit) VALUES ('Horchposten','1','$lastid','1','$fid','100','40','40','30','30','0','noname','$accid','green','".$schiff->position->x."','".$schiff->position->y."','".$schiff->position->system->id."','10','10','5','images/horchposten.png','150','".$schiff->position->orbit."')") or die(mysql_error());
echo 'Schiff gebaut!';
$gebaut=true;
} else echo 'Nicht genug Rohstoffe!';
}
...
mysql_query("INSERT INTO schiffe (maxdock,maxphaser,typ,klasse,skillbase,baustoff,duranium,deuterium,hull,maxhull,schilde,maxschilde,laser,name,besitzer,alarmstufe,x,y,system,energie,maxenergie,energieoutput,bild,lager,orbit) VALUES ('20','50','s','Raumstation','1','0','0','100','400','400','500','500','22','noname','$accid','green','".$schiff->position->x."','".$schiff->position->y."','".$schiff->position->system->id."','500','500','50','images/ferbase.png','15000','".$schiff->position->orbit."')") or die(mysql_error());
echo 'Schiff gebaut!';
$gebaut=true;
}

if($job==7) { 	//Schiff 3 bauen
mysql_query("INSERT INTO schiffe (maxdock,maxphaser,typ,klasse,skillbase,baustoff,duranium,deuterium,hull,maxhull,schilde,maxschilde,laser,name,besitzer,alarmstufe,x,y,system,energie,maxenergie,energieoutput,bild,lager,orbit) VALUES ('20','50','s','Raumstation','1','0','0','100','400','400','500','500','22','noname','$accid','green','".$schiff->position->x."','".$schiff->position->y."','".$schiff->position->system->id."','500','500','50','images/cardbase.png','15000','".$schiff->position->orbit."')") or die(mysql_error());
echo 'Schiff gebaut!';
$gebaut=true;
}

if($job==8) { 	//Schiff 3 bauen
mysql_query("INSERT INTO schiffe (maxdock,maxphaser,typ,klasse,skillbase,baustoff,duranium,deuterium,hull,maxhull,schilde,maxschilde,laser,name,besitzer,alarmstufe,x,y,system,energie,maxenergie,energieoutput,bild,lager,orbit) VALUES ('20','50','s','Raumstation','1','0','0','100','400','400','500','500','22','noname','$accid','green','".$schiff->position->x."','".$schiff->position->y."','".$schiff->position->system->id."','500','500','50','images/borgbase.gif','15000','".$schiff->position->orbit."')") or die(mysql_error());
echo 'Schiff gebaut!';
$gebaut=true;
}
...
if($_SESSION["Id"]>9) echo '<td>Station</td><td><img src="images/siedlerbase.png" border="0"></td><td>600 Baustoff<br />250 Duranium<br />10 Sorium</td><td>140</td><td>200</td><td>12</td><td>0</td><td>0</td><td>0</td><td>300</td><td>3</td><td>optimale Verteidigungsstation</td><td><a href="base.php?sid=',$schiff->id,'&job=1">Basis bauen!</a></td></tr>';
if($forschung->horchposten==1) echo '<td>Horchposten</td><td><img src="images/horchposten.png" border="0"></td><td>200 Baustoff<br />150 Duranium<br />200 Deuterium</td><td>40</td><td>30</td><td>0</td><td>0</td><td>0</td><td>0</td><td>40</td><td>1</td><td>&Uuml;berwacht ein Gebiet von Radius 5 um die Station, also ein 10x10 Feld</td><td><a href="base.php?sid=',$schiff->id,'&job=10">Horchposten bauen!</a></td></tr>';
...
if($_SESSION["Id"]==4 || $_SESSION["Id"]==1) echo '<td>Station</td><td><img src="images/borgbase.gif" border="0"></td><td>-</td><td>10000</td><td>20000</td><td>500</td><td>0</td><td>0</td><td>0</td><td>15000</td><td>0</td><td>optimale Verteidigungsstation</td><td><a href="base.php?sid=',$schiff->id,'&job=8">Basis bauen!</a></td></tr>';
