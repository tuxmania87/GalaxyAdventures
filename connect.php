...
{
	$verbindung = mysqli_connect("localhost", "gauser", "gauser82859415");

	mysqli_query($verbindung, "SET NAMES 'utf8'") or die($verbingung->mysql_error());
	mysqli_query($verbindung, "SET character_set_connection = 'utf8'") or die($verbingung->mysql_error());
	mysqli_query($verbindung, "SET character_set_database = 'utf8'") or die($verbingung->mysql_error());
	mysqli_query($verbindung, "SET character_set_server = 'utf8'") or die($verbingung->mysql_error());
...
function isonline($aktion)
{
	...
	if ($tmpvar1 == date("Y-m-d")) {
		...
		$tmpvar3 += $tmpvar4;
		if ($tmpvar3 >= $tmpvar2 - 240 && $tmpvar3 <= $tmpvar2) return true;
		else return false;
	}
}
...
function getSlot($id)
{
	...
	while ($schiff = mysqli_fetch_array($abfrage)) {
		...
		if ($schiff["klasse"] == 'Oberth') $count += 1.5;
		if ($schiff["klasse"] == 'Miranda') $count += 3;
		if ($schiff["klasse"] == 'Constitution') $count += 5;
		if ($schiff["skillbase"] == 1) $count += 4;
	}
	return $count;
}
...
function splitfeld($text, &$was, &$bauzeit, &$auf, &$hull, &$aktiv)
{
	...
	for ($i = 0; $i < strlen($text); $i++) {
		...
		if ($text[$i] == '-') {
			$weiter = false;
			if (!$eins) $eins = true;
			else if (!$zwei) $zwei = true;
			else if (!$drei) $drei = true;
			else if (!$vier) $vier = true;
		}

		if ($weiter) {
			if ($vier) $aktiv .= $text[$i];
			else
if ($drei) $hull .= $text[$i];
			else
if ($zwei) $auf .= $text[$i];
			else
if ($eins) $bauzeit .= $text[$i];
			else
				$was .= $text[$i];
		}
	}
...
$selfid = session_id();
if (session_id() != "") {
	mysqli_query($verbindung, "UPDATE account SET aktion='$action' WHERE id='" . $selfid . "'");
}
